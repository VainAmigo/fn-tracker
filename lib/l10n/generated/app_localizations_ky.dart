// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Kirghiz Kyrgyz (`ky`).
class AppLocalizationsKy extends AppLocalizations {
  AppLocalizationsKy([String locale = 'ky']) : super(locale);

  @override
  String get signIn => 'Кирүү';

  @override
  String get signUp => 'Катталуу';

  @override
  String get january => 'Январь';

  @override
  String get february => 'Февраль';

  @override
  String get march => 'Март';

  @override
  String get april => 'Апрель';

  @override
  String get may => 'Май';

  @override
  String get june => 'Июнь';

  @override
  String get july => 'Июль';

  @override
  String get august => 'Август';

  @override
  String get september => 'Сентябрь';

  @override
  String get october => 'Октябрь';

  @override
  String get november => 'Ноябрь';

  @override
  String get december => 'Декабрь';

  @override
  String get januaryShort => 'Янв';

  @override
  String get februaryShort => 'Фев';

  @override
  String get marchShort => 'Мар';

  @override
  String get aprilShort => 'Апр';

  @override
  String get mayShort => 'Май';

  @override
  String get juneShort => 'Июн';

  @override
  String get julyShort => 'Июл';

  @override
  String get augustShort => 'Авг';

  @override
  String get septemberShort => 'Сен';

  @override
  String get octoberShort => 'Окт';

  @override
  String get novemberShort => 'Ноя';

  @override
  String get decemberShort => 'Дек';

  @override
  String get monday => 'Дүйшөмбү';

  @override
  String get tuesday => 'Шейшемби';

  @override
  String get wednesday => 'Шаршемби';

  @override
  String get thursday => 'Бейшемби';

  @override
  String get friday => 'Жума';

  @override
  String get saturday => 'Ишемби';

  @override
  String get sunday => 'Жекшемби';

  @override
  String get mondayShort => 'Дүй';

  @override
  String get tuesdayShort => 'Шей';

  @override
  String get wednesdayShort => 'Шар';

  @override
  String get thursdayShort => 'Бей';

  @override
  String get fridayShort => 'Жум';

  @override
  String get saturdayShort => 'Ишм';

  @override
  String get sundayShort => 'Жек';

  @override
  String get week => 'Жума';

  @override
  String get threeMonths => '3 ай';

  @override
  String get sixMonths => '6 ай';

  @override
  String get expense => 'Чыгым';

  @override
  String get income => 'Киреше';

  @override
  String get transfer => 'Которуу';

  @override
  String get deleteEntityCancel => 'Жокко чыгаруу';

  @override
  String get deleteEntityPartial => 'Жарым-жартылай өчүрүү';

  @override
  String get deleteEntityFull => 'Толугу менен өчүрүү';

  @override
  String get deleteEntityPartialHint =>
      'Жарым-жартылай өчүрүү — максат/капчыкты өчүрүү, транзакциялар сакталат.';

  @override
  String get deleteEntityFullHint =>
      'Толугу менен өчүрүү — баардык байланышкан транзакциялар менен бирге өчүрүү.';

  @override
  String get noTransactions => 'Транзакциялар жок';

  @override
  String get noTransactionsSubtitle => 'Сизде азырынча транзакциялар жок';

  @override
  String get exportColumnDate => 'Дата';

  @override
  String get createdAt => 'Түзүлгөн';

  @override
  String get exportColumnType => 'Түрү';

  @override
  String get amount => 'Сумма';

  @override
  String get exportColumnCurrency => 'Валюта';

  @override
  String get exportColumnCategory => 'Категория';

  @override
  String get exportColumnWallet => 'Капчык';

  @override
  String get note => 'Эскертүү';

  @override
  String get exportColumnTransactionId => 'Транзакция ID';

  @override
  String get exportColumnDateDescription =>
      'Колдонуучу тандаган операция датасы.';

  @override
  String get exportColumnCreatedAtDescription =>
      'Маалымат базасында жазуунун түзүлгөн датасы.';

  @override
  String get exportColumnTypeDescription =>
      'Операция түрү: чыгым, киреше же которуу.';

  @override
  String get exportColumnAmountDescription =>
      'Операция суммасы эки ондук тактык менен.';

  @override
  String get exportColumnCurrencyDescription =>
      'Операцияда колдонулган валюта коду.';

  @override
  String get exportColumnCategoryDescription =>
      'Операцияга байланышкан категориянын аталышы.';

  @override
  String get exportColumnWalletDescription =>
      'Операцияга байланышкан капчыктын аталышы.';

  @override
  String get exportColumnNoteDescription =>
      'Операцияга колдонуучунун эскертүүсү.';

  @override
  String get exportColumnTransactionIdDescription =>
      'Операциянын уникалдуу идентификатору.';

  @override
  String get startTakingControlOfYourFinances =>
      'Финанстарды башкарууга баштаңыз';

  @override
  String get continueWithEmail => 'Электрондук почта менен кириңиз';

  @override
  String get noAccount => 'Аккаунтум жок? Катталуу';

  @override
  String get alreadyHaveAccount => 'Аккаунтум бар? Кирүү';

  @override
  String get register => 'Катталуу';

  @override
  String get login => 'Кирүү';

  @override
  String get continueWithGoogle => 'Google менен кириңиз';

  @override
  String get email => 'Электрондук почта';

  @override
  String get password => 'Сырсөз';

  @override
  String get confirmPassword => 'Сырсөздү тастыктоо';

  @override
  String get enterYourEmail => 'Электрондук почтаны киргизиңиз';

  @override
  String get enterYourPassword => 'Сырсөздү киргизиңиз';

  @override
  String get confirmYourPassword => 'Сырсөздү тастыктаңыз';

  @override
  String get invalidEmail => 'Туура эмес email';

  @override
  String passwordMustBeAtLeast(int minLength) {
    return 'Сырсөз ең аз $minLength символдон турушу керек';
  }

  @override
  String get passwordsDoNotMatch => 'Сырсөздөр дал келбейт';

  @override
  String get spent => 'Коротулду';

  @override
  String get quickCategories => 'Тез категориялар';

  @override
  String get lastTransactions => 'Акыркы транзакциялар';

  @override
  String get viewAll => 'Баарын көрүү';

  @override
  String get settings => 'Жөндөөлөр';

  @override
  String get homeWalletsSettingsSubtitle =>
      'Башкы экранда кайсы капчыктар көрүнөрүн тандаңыз';

  @override
  String get showOnHome => 'Башкыда көрсөтүү';

  @override
  String get home => 'Башкы';

  @override
  String get finance => 'Финансы';

  @override
  String get analytics => 'Аналитика';

  @override
  String get chooseHowCategoriesAreDisplayed =>
      'Башкы экранда категориялар кантип көрсөтүлөрүн тандаңыз';

  @override
  String get recent => 'Акыркы';

  @override
  String get pinned => 'Бекитилген';

  @override
  String get showCategoriesFromYourLastTransactions =>
      'Акыркы транзакцияларыңыздан категорияларды көрсөтүү';

  @override
  String get showCategoriesFromYourQuickCategories =>
      'Тез жетүү үчүн бекиткен категорияларыңызды көрсөтүү';

  @override
  String get homeScreenWidgetSource => 'Башкы экран виджетинин булагы';

  @override
  String get systemQuickCategories => 'Системалык тез категориялар';

  @override
  String get userQuickCategories =>
      'Учурдагы тез режимдеги колдонуучу категориялары';

  @override
  String get customCategories => 'Жеке категориялар';

  @override
  String get chooseFixedCategories =>
      'Башкы экран үчүн категориялардын туруктуу тизмесин тандаңыз';

  @override
  String get available => 'Жеткиликтүү';

  @override
  String get createCategoriesFirst => 'Алгач категорияларды жасаңыз';

  @override
  String get pinnedCategories => 'Бекиткен категориялар';

  @override
  String get selectCategoriesToShowInQuickAccess =>
      'Башкы экранда тез жетүү үчүн категорияларын тандаңыз';

  @override
  String get save => 'Сактоо';

  @override
  String get widgetCategories => 'Виджет категориялары';

  @override
  String get selectCustomCategoriesForTheHomeScreenWidget =>
      'Башкы экран виджети үчүн жеке категорияларын тандаңыз';

  @override
  String get transactions => 'Транзакциялар';

  @override
  String get category => 'Категория';

  @override
  String get description => 'Түшүндүрүү';

  @override
  String get to => 'Каяка';

  @override
  String get from => 'Каяктан';

  @override
  String get date => 'Дата';

  @override
  String get manageYourAccountAndPreferences =>
      'Аккаунтуңузду жана параметрлерди башкарыңыз';

  @override
  String get appSettings => 'Колдонмо жөндөөлөрү';

  @override
  String get appTheme => 'Колдонмонун темасы';

  @override
  String get language => 'Тил';

  @override
  String get currencyAndFormats => 'Валюта жана форматтар';

  @override
  String get privacy => 'Купуялык';

  @override
  String get privacyPolicy => 'Купуялык саясаты';

  @override
  String get security => 'Коопсуздук';

  @override
  String get signOut => 'Чыгуу';

  @override
  String get signOutConfirmation => 'Аккаунттан чыгасызбы?';

  @override
  String get cancel => 'Жокко чыгаруу';

  @override
  String get selectYourCurrencyAndNumberFormat =>
      'Валюта жана сан форматын тандаңыз';

  @override
  String get dynamicColorsOfTheDevice => 'Түзмөктүн динамикалык түстөрү';

  @override
  String get dynamicColorsOfTheDeviceDescription =>
      'Android 12+де тема түзмөккө ылайыкталат. Башка платформаларда же түстөр жеткиликтүү эмес болсо, төмөнкү палитра колдонулат.';

  @override
  String get system => 'Тутум';

  @override
  String get dark => 'Караңгы';

  @override
  String get light => 'Жарык';

  @override
  String get themeMode => 'Тема режими';

  @override
  String get theme => 'Тема';

  @override
  String get addTransaction => 'Транзакция кошуу';

  @override
  String get amountCannotBeEmpty => 'Сумма бош болбошу керек';

  @override
  String get amountMustBeGreaterThanZero => 'Сумма нөлдөн чоң болушу керек';

  @override
  String get selectBothSourceAndDestinationAccounts =>
      'Баштапкы жана максаттуу эсептерди экөөн тең тандаңыз';

  @override
  String get sourceAndDestinationMustBeDifferent =>
      'Баштапкы жана максаттуу эсептер ар башка болушу керек';

  @override
  String get selectWalletOrGoal => 'Капчык же максатты тандаңыз';

  @override
  String get categoryCannotBeEmpty => 'Категория бош болбошу керек';

  @override
  String get addNote => 'Эскертүү кошуу';

  @override
  String get enterYourNote => 'Эскертүүңүздү киргизиңиз';

  @override
  String get wallet => 'Капчык';

  @override
  String get today => 'Бүгүн';

  @override
  String get yesterday => 'Кечээ';

  @override
  String get goal => 'Максат';

  @override
  String get chooseCategory => 'Категорияны тандаңыз';

  @override
  String get selectDate => 'Датаны тандаңыз';

  @override
  String get chooseDate => 'Датаны тандаңыз';

  @override
  String get chooseAccount => 'Эсепти тандаңыз';

  @override
  String get yourWallets => 'Капчыктарыңыз';

  @override
  String get yourGoals => 'Максаттарыңыз';

  @override
  String get cannotSelectSameAccount => 'Ошол эле эсепти кайра тандап болбойт';

  @override
  String get addWithVoice => 'Үн менен кошуу';

  @override
  String get addWithFile => 'Файл менен кошуу';

  @override
  String get addManually => 'Кол менен кошуу';

  @override
  String get voice => 'Үн';

  @override
  String get file => 'Файл';

  @override
  String get addTransactionWithAi => 'ИИ менен транзакция кошуу';

  @override
  String get enterTransactionsManuallyOrUseTemplates =>
      'Транзакцияларды кол менен киргизиңиз же үлгүлөрдү колдонуңуз';

  @override
  String get attachment => 'Тиркеме';

  @override
  String get camera => 'Камера';

  @override
  String get gallery => 'Галерея';

  @override
  String get removeFile => 'Файлды өчүрүү';

  @override
  String get textCanBeEdited => 'Текст өзгөртүлүшү мүмкүн';

  @override
  String get cancelAndReturnToInput => 'Жокко чыгарып, киргизүүгө кайтуу';

  @override
  String get saveAll => 'Баарын сактоо';

  @override
  String get returnToInput => 'Киргизүүгө кайтуу';

  @override
  String get listening => 'Угулууда...';

  @override
  String get listen => 'Угуу';

  @override
  String get photo => 'Сүрөт';

  @override
  String get pdf => 'PDF';

  @override
  String get processing => 'Иштетилүүдө...';

  @override
  String get recognize => 'Таануу';

  @override
  String get upTo5Words => '5 сөзгө чейин';

  @override
  String get yourFinancesAndSavings => 'Финанстар жана чогултууларыңыз';

  @override
  String get budget => 'Бюджет';

  @override
  String get accounts => 'Эсептер';

  @override
  String get categories => 'Категориялар';

  @override
  String get scheduledPayments => 'Пландаштырылган төлөмдөр';

  @override
  String get editBudget => 'Бюджетти оңдоо';

  @override
  String get createBudget => 'Бюджет түзүү';

  @override
  String get noBudgetFound => 'Бюджет табылган жок. Алгач түзүңүз.';

  @override
  String get somethingWentWrong =>
      'Бир нерсе туура эмес кетти. Кийинчерээк кайра аракет кылыңыз.';

  @override
  String get retry => 'Кайра аракет';

  @override
  String get budgetDetails => 'Бюджет тууралуу';

  @override
  String get delete => 'Өчүрүү';

  @override
  String get deleteBudget => 'Бюджетти өчүрүү?';

  @override
  String get deleteBudgetConfirmation => 'Бул бюджетти өчүрүүнү каалайсызбы?';

  @override
  String get noHistoryEntries => 'Тарых жазуулары жок';

  @override
  String get editHistoryEntry => 'Тарых жазуусун оңдоо';

  @override
  String get deleteHistoryEntry => 'Тарых жазуусун өчүрүү?';

  @override
  String get budgetExceeded => 'Бюджет ашып кетти';

  @override
  String get remaining => 'калды';

  @override
  String get overspent => 'Ашып кетти';

  @override
  String get replaceAll => 'Баарын алмаштыруу';

  @override
  String get fromDate => 'Баштоо датасы';

  @override
  String get effectiveFrom => 'Күчүнө кирүү';

  @override
  String get info => 'Маалымат';

  @override
  String get aboutBudget => 'Бюджет тууралуу';

  @override
  String get budgetDescription =>
      'Бюджет — айлык чыгым чеги. Ага каршы канча короткондугуңузду карап, бюджет өзгөргөндө жаңы суммалар кошсоңуз болот.';

  @override
  String get addOptions => 'Кошуу параметрлери';

  @override
  String get replaceAllDescription =>
      'Бюджет тарыхын жаңы сумма менен толугу менен алмаштырат. Бюджетти толугу менен кайра коюуну каалаганда колдонуңуз.';

  @override
  String get fromDateDescription =>
      'Белгилүү датадан баштап күчүнө кирген жаңы бюджет суммасын кошот. Мурунку жазуулар тарыхта калат.';

  @override
  String get yearlyBudget => 'Жылдык бюджет';

  @override
  String get monthlyBudget => 'Айлык бюджет';

  @override
  String get weeklyBudget => 'Апталик бюджет';

  @override
  String get budgetCategories => 'Бюджет категориялары';

  @override
  String get limitExceeded => 'Чек ашылды';

  @override
  String get wallets => 'Капчыктар';

  @override
  String get createWallet => 'Капчык түзүү';

  @override
  String get newGoal => 'Жаңы максат';

  @override
  String get changePin => 'PIN өзгөртүү';

  @override
  String get pinSuccessfullyChanged => 'PIN ийгиликтүү өзгөртүлдү';

  @override
  String get setPin => 'PIN коюу';

  @override
  String get pinRequiredForHiddenCards =>
      'Жашыруу карталар үчүн PIN талап кылынат';

  @override
  String get hiddenCards => 'Жашыруу карталар';

  @override
  String get enterPinToView => 'Көрүү үчүн PIN киргизиңиз';

  @override
  String get open => 'Ачуу';

  @override
  String get invalidPin => 'Туура эмес PIN';

  @override
  String get goalUpdatedSuccessfully => 'Максат ийгиликтүү жаңыланды';

  @override
  String get goalCreatedSuccessfully => 'Максат ийгиликтүү түзүлдү';

  @override
  String get updateGoal => 'Максатты жаңылоо';

  @override
  String get createGoal => 'Максат түзүү';

  @override
  String get goalName => 'Максаттын аты';

  @override
  String get targetAmount => 'Максат суммасы';

  @override
  String get howMuchDoYouWantToSave => 'Канча үнөмдөмөксүз?';

  @override
  String get progress => 'Прогресс';

  @override
  String get enterValidTargetAmount => 'Туура максат суммасын киргизиңиз';

  @override
  String get nameIsRequired => 'Аталышы милдеттүү';

  @override
  String get walletUpdatedSuccessfully => 'Капчык ийгиликтүү жаңыланды';

  @override
  String get walletCreatedSuccessfully => 'Капчык ийгиликтүү түзүлдү';

  @override
  String get updateWallet => 'Капчыкты жаңылоо';

  @override
  String get walletName => 'Капчыктын аты';

  @override
  String get completed => 'Аяктады';

  @override
  String get complete => 'Аяктоо';

  @override
  String get goalDetailsTitle => 'Максат тууралуу';

  @override
  String get goalAmount => 'Максат суммасы';

  @override
  String get completedAmount => 'Аякталган сумма';

  @override
  String get completedAt => 'Аякталган дата';

  @override
  String get history => 'Тарых';

  @override
  String get deleteGoalTitle => 'Максатты өчүрүү?';

  @override
  String get deleteGoalMessage => 'Бул максатты өчүрүүнү каалайсызбы?';

  @override
  String get edit => 'Оңдоо';

  @override
  String get hideAmount => 'Сумманы жашыруу';

  @override
  String get hideGoal => 'Максатты жашыруу';

  @override
  String get hideGoalSubtitle => '«Жашыруу карталар» блокунда гана көрүнөт';

  @override
  String get deleteGoalMessageHint => 'Максатты өчүрүү жолун тандаңыз';

  @override
  String get deposit => 'Кошуу';

  @override
  String get noGoals => 'Азырынча максат жок';

  @override
  String get card => 'карта';

  @override
  String get cards => 'карталар';

  @override
  String get inProgress => 'Жүрүп жатат';

  @override
  String get totalProgress => 'Жалпы прогресс';

  @override
  String get goals => 'Максаттар';

  @override
  String get active => 'Активдүү';

  @override
  String get walletDetails => 'Капчык тууралуу';

  @override
  String get hideWallet => 'Капчыкты жашыруу';

  @override
  String get hideWalletSubtitle => '«Жашыруу карталар» блокунда гана көрүнөт';

  @override
  String get deleteWalletTitle => 'Капчыкты өчүрүү?';

  @override
  String get deleteWalletMessage => 'Бул капчыкты өчүрүүнү каалайсызбы?';

  @override
  String get deleteWalletMessageHint => 'Капчыкты өчүрүү жолун тандаңыз';

  @override
  String get noWallets => 'Азырынча капчык жок';

  @override
  String get createYourFirstWallet => 'Биринчи капчыгыңызды түзүңүз';

  @override
  String get newWallet => 'Жаңы капчык';

  @override
  String get categoryDetails => 'Категория тууралуу';

  @override
  String get createCategory => 'Категория түзүү';

  @override
  String get limit => 'Чек';

  @override
  String get deleteCategoryTitle => 'Категорияны өчүрүү?';

  @override
  String get deleteCategoryMessage => 'Бул категорияны өчүрүүнү каалайсызбы?';

  @override
  String get deleteCategoryMessageHint => 'Категорияны өчүрүү жолун тандаңыз';

  @override
  String get scheduledPayment => 'Пландаштырылган төлөм';

  @override
  String get paymentName => 'төлөмдүн аты';

  @override
  String get paymentAmountDescription => 'Канча төлөмөксүз?';

  @override
  String get frequency => 'Жыштык';

  @override
  String get paymentDate => 'Төлөм күнү';

  @override
  String get scheduledPaymentType => 'Пландаштырылган төлөм түрү';

  @override
  String get nextPayment => 'Кийинки төлөм';

  @override
  String get walletOrGoal => 'Капчык же максат';

  @override
  String get selectWallet => 'Капчыкты тандаңыз';

  @override
  String get selectCategory => 'Категорияны тандаңыз';

  @override
  String get enableAutoPayment => 'Авто-төлөмдү күйгүзүү';

  @override
  String get enterName => 'Ат киргизиңиз';

  @override
  String get enterAmount => 'Сумманы киргизиңиз';

  @override
  String get selectDateOrDays => 'Дата же күндөрдү тандаңыз';

  @override
  String get paymentDateMustBeInTheFuture =>
      'Төлөм датасы келечекте болушу керек';

  @override
  String get once => 'Бир жолу';

  @override
  String get monthly => 'Ай сайын';

  @override
  String get yearly => 'Жыл сайын';

  @override
  String get weekly => 'Апта сайын';

  @override
  String get endOfMonth => 'Айдын аягы';

  @override
  String get subscriptions => 'Жазылуулар';

  @override
  String get regularPayments => 'Туруктуу төлөмдөр';

  @override
  String get regularIncomePayments => 'Туруктуу киреше төлөмдөрү';

  @override
  String get repeat => 'Кайталоо';

  @override
  String get paymentsOn => 'Төлөмдөр';

  @override
  String get add => 'Кошуу';

  @override
  String get paymentFrequency => 'Төлөм жыштыгы';

  @override
  String get paused => 'Токтотулган';

  @override
  String get done => 'Бүттү';

  @override
  String get daysOfMonth => 'Айдын күндөрү';

  @override
  String get dayOfMonth => 'Айдын күнү';

  @override
  String get paymentDatesInYear => 'Жыл ичиндеги төлөм даталары';

  @override
  String get dateMonthDay => 'Дата (ай жана күн)';

  @override
  String get selectDateForOneTimePayment =>
      'Бир жолку төлөм үчүн датаны тандаңыз';

  @override
  String get paymentWillBeMadeOnSelectedDaysOfMonthEveryMonth =>
      'Ай сайын тандалган күндөрдө төлөм жүргүзүлөт';

  @override
  String get paymentWillBeMadeOnSelectedDayOfMonthEveryMonth =>
      'Ай сайын тандалган айдын күнүндө төлөм жүргүзүлөт';

  @override
  String get paymentWillBeMadeOnSelectedDatesEveryYear =>
      'Жыл сайын тандалган даталарда төлөм жүргүзүлөт';

  @override
  String get paymentWillBeMadeOnSelectedDateMonthDayEveryYear =>
      'Жыл сайын тандалган датада (ай жана күн) төлөм жүргүзүлөт';

  @override
  String get first => 'Биринчи';

  @override
  String get second => 'Экинчи';

  @override
  String get third => 'Үчүнчү';

  @override
  String get th => 'чи';

  @override
  String get selectedDays => 'Тандалган күндөр';

  @override
  String get pressToSelectDays => 'Күн тандоо үчүн басыңыз';

  @override
  String get chooseMonthAndDay => 'Ай жана күн тандаңыз';

  @override
  String get addDate => 'Дата кошуу';

  @override
  String get editScheduledPayment => 'Пландаштырылган төлөмдү оңдоо';

  @override
  String get paid => 'Төлөндү';

  @override
  String get autoCreateTransaction => 'Транзакцияны автоматтык түзүү';

  @override
  String get pause => 'Токтотуу';

  @override
  String get pay => 'Төлөө';

  @override
  String get typeOfScheduledPayment => 'Пландаштырылган төлөмдүн түрү';

  @override
  String get createScheduledPayment => 'Пландаштырылган төлөм түзүү';

  @override
  String get categoryDeleted => 'Категория өчүрүлдү';

  @override
  String get categoryUpdatedSuccessfully => 'Категория ийгиликтүү жаңыланды';

  @override
  String get categoryCreatedSuccessfully => 'Категория ийгиликтүү түзүлдү';

  @override
  String get updateCategory => 'Категорияны жаңылоо';

  @override
  String get categoryName => 'Категориянын аты';

  @override
  String get noLimit => 'Чек жок';

  @override
  String get monthlyLimit => 'Айлык чек';

  @override
  String get update => 'Жаңылоо';

  @override
  String get create => 'Түзүү';

  @override
  String get noCategories => 'Категориялар жок';

  @override
  String get createYourFirstCategory => 'Биринчи категорияңызды түзүңүз';

  @override
  String get newCategory => 'Жаңы категория';

  @override
  String get icon => 'Сүрөтчө';

  @override
  String get exportSettings => 'Экспорт жөндөөлөрү';

  @override
  String get period => 'Мезгил';

  @override
  String get month => 'Ай';

  @override
  String get custom => 'Жеке';

  @override
  String get selectedColumns => 'Тандалган устундар';

  @override
  String get settingsSaved => 'Жөндөөлөр сакталды';

  @override
  String get export => 'Экспорт';

  @override
  String get exportFileIsReady => 'Экспорт файлы даяр';

  @override
  String get fileCreatedButShareDialogIsUnavailableOnThisDevice =>
      'Файл түзүлдү, бирок бул түзмөктө бөлүшүү диалогу жеткиликтүү эмес';

  @override
  String get noDataForSelectedPeriod => 'Тандалган мезгил үчүн маалымат жок';

  @override
  String get choosePeriodInExportSettings =>
      'Экспорт жөндөөлөрүндө мезгилди тандаңыз';

  @override
  String get failedToExportData => 'Маалыматты экспорттоо ишке ашкан жок';

  @override
  String get unknownError => 'Белгисиз ката';

  @override
  String get customPeriod => 'Жеке мезгил';

  @override
  String get deepAnalytics => 'Терең аналитика';

  @override
  String get askAboutYourAnalyticsForThisPeriod =>
      'Бул мезгилдеги аналитикаңыз жөнүндө сураңыз';

  @override
  String get clearChat => 'Чатты тазалоо';

  @override
  String get askAboutYourAnalyticsForThisPeriodHint =>
      'Аналитикаңызды сураңыз…';

  @override
  String get aiAssistant => 'ИИ жардамчысы';

  @override
  String get spendingChart => 'Чыгым диаграммасы';

  @override
  String get balance => 'Баланс';

  @override
  String get enterCurrentPin => 'Учурдагы PIN киргизиңиз';

  @override
  String get enterNewPin => 'Жаңы PIN киргизиңиз';

  @override
  String get newPinDoesNotMatch => 'Жаңы PIN дал келбейт';

  @override
  String get newPinMustBeDifferent => 'Жаңы PIN башка болушу керек';

  @override
  String get invalidCurrentPin => 'Учурдагы PIN туура эмес';

  @override
  String get currentPin => 'Учурдагы PIN';

  @override
  String get newPin => 'Жаңы PIN';

  @override
  String get confirmNewPin => 'Жаңы PINди тастыктаңыз';

  @override
  String get confirm => 'Тастыктоо';

  @override
  String get pinDoesNotMatch => 'PIN дал келбейт';

  @override
  String get enterPin => 'PIN киргизиңиз';

  @override
  String get confirmPin => 'PINди тастыктаңыз';

  @override
  String get aiGoalAdviceTitle => 'ИИ топтоо кеңештери';

  @override
  String get aiGoalAdviceSingleGoalHint =>
      'Өз маалыматтарыңызга таянып ушул максатка тезирээк топтоо жолун сураңыз.';

  @override
  String get aiGoalAdviceAllGoalsHint =>
      'Бардык максаттар боюнча топтоону кантип жакшыраак бөлүштүрүүнү сураңыз.';

  @override
  String get aiGoalAdviceQuestionHint =>
      'Мисалы: максатка тез жетүү үчүн эмнени биринчи кыскартсам болот?';

  @override
  String get aiGoalAdviceAsk => 'Кеңеш алуу';

  @override
  String get aiGoalAdviceTotalAsk => 'Бардык максаттар боюнча ИИ кеңештер';

  @override
  String get aiGoalAdviceTotalTitle => 'Бардык максаттар боюнча ИИ кеңештер';

  @override
  String get aiGoalAdviceDefaultQuestionAll =>
      'Бардык максаттарымды, капчыктарымды, категорияларымды жана транзакцияларымды талдап бер. Кыска план бер: кайсы максаттарды артыкчылыкка коюу керек, кайсы чыгымдарды биринчи кыскартуу керек жана жалпы топтоону кантип ылдамдатсам болот.';

  @override
  String aiGoalAdviceDefaultQuestionSingle(Object goalName) {
    return 'Бардык маалыматтарымды талдап, \"$goalName\" максатына тезирээк жетүү үчүн практикалык кеңеш бер: кайсы чыгымдарды биринчи кыскартуу керек жана канча өлчөмдө туруктуу топтоп туруу керек.';
  }

  @override
  String get pin => 'PIN';

  @override
  String get authErrorGeneric => 'Ката кетти. Кайра аракет кылыңыз.';

  @override
  String get authErrorAccountExistsWithDifferentCredential =>
      'Бул электрондук почта менен аккаунт бар, бирок башка кирүү ыкмасы менен.';

  @override
  String get authErrorCredentialAlreadyInUse =>
      'Бул Google аккаунты башка колдонуучуга байланышкан.';

  @override
  String get authErrorProviderAlreadyLinked =>
      'Google буга чейин бул аккаунтка байланышкан.';

  @override
  String get authErrorRequiresRecentLogin =>
      'Коопсуздук үчүн чыгып, кайра кирип, анан Google байлаңыз.';

  @override
  String get authErrorInvalidCredential =>
      'Туура эмес маалымат. Google аккаунтуңузду текшериңиз же кайра аракет кылыңыз.';

  @override
  String get authErrorUserDisabled => 'Бул аккаунт өчүрүлгөн.';

  @override
  String get authErrorNetwork => 'Тармак катасы. Туташууну текшериңиз.';

  @override
  String get authErrorNotSignedIn => 'Сиз аккаунтка кирген жоксуз.';

  @override
  String get authErrorGoogleSignIn =>
      'Google аркылуу кирүү ишке ашкан жок. Firebase Console жөндөөлөрүн текшериңиз.';

  @override
  String get linkGoogleTitle => 'Google байлоо';

  @override
  String get linkGoogleSubtitle =>
      'Ошол эле профиль менен бул түзмөктө Google аркылуу кирүү';

  @override
  String get googleAlreadyLinkedTitle => 'Google байланган';

  @override
  String get googleAlreadyLinkedSubtitle =>
      'Сиз бул түзмөктө Google аркылуу кире аласыз';

  @override
  String get googleLinkedSuccessfully => 'Google аккаунты ийгиликтүү байланды';

  @override
  String get displayNameLabel => 'Көрсөтүлгөн ат';

  @override
  String get displayNameNotSet => 'Коюлган жок';

  @override
  String get displayNameTapToSet => 'Атты өзгөртүү үчүн басыңыз';

  @override
  String get displayNameWhenSet => 'Колдонмодогу ат';

  @override
  String get editDisplayNameTitle => 'Атты өзгөртүү';

  @override
  String get displayNameUpdated => 'Ат жаңыланды';

  @override
  String get displayNameTooLong => 'Ат өтө узун (максимум 120 символ)';

  @override
  String get deleteUserDataSectionTitle => 'Маалымат жана аккаунт';

  @override
  String get deleteUserDataButton => 'Бардык маалыматты өчүрүү';

  @override
  String get deleteUserDataTitle => 'Бардык маалыматты өчүрүү?';

  @override
  String get deleteUserDataMessage =>
      'Колдонмодогу бардык транзакциялар, капчыктар, максаттар, категориялар, бюджеттер жана жөндөөлөр түбөлүк өчүрүлөт. Кирүү жана профиль (почта, ат) сакталат.';

  @override
  String get deleteUserDataConfirm => 'Маалыматты өчүрүү';

  @override
  String get deleteAccountButton => 'Аккаунтту өчүрүү';

  @override
  String get deleteAccountTitle => 'Аккаунтту өчүрүү?';

  @override
  String get deleteAccountMessage =>
      'Аккаунт жана бардык маалыматтар түбөлүк өчүрүлөт. Бул аракатты жокко чыгарбайсыз.';

  @override
  String get deleteAccountConfirm => 'Аккаунтту өчүрүү';

  @override
  String get enterPasswordToConfirmDelete => 'Ырастоо үчүн сырсөздү киргизиңиз';

  @override
  String get continueWithGoogleToConfirmDelete =>
      'Ырастоо үчүн Google менен кириңиз';

  @override
  String get userDataDeleted => 'Бардык маалымат өчүрүлдү';

  @override
  String get accountDeleted => 'Аккаунт өчүрүлдү';

  @override
  String get changePassword => 'Сырсөздү өзгөртүү';

  @override
  String get changePasswordCardSubtitle => 'Сырсөздү өзгөртүү үчүн басыңыз';

  @override
  String get currentPassword => 'Учурдагы сырсөз';

  @override
  String get newPassword => 'Жаңы сырсөз';

  @override
  String get enterYourCurrentPassword => 'Учурдагы сырсөздү киргизиңиз';

  @override
  String get passwordChangedSuccessfully => 'Сырсөз ийгиликтүү өзгөртүлдү';

  @override
  String get passwordChangeEmailOnly =>
      'Сырсөздү өзгөртүү электрондук почта менен кирүүдө гана жеткиликтүү';

  @override
  String get account => 'Аккаунт';

  @override
  String get accountTitle => 'Сиздин аккаунтыңыз';

  @override
  String get accountSubtitle => 'Сиздин аккаунтыңызды башкаруу';

  @override
  String get accountSettings => 'Аккаунттын башкаруу';

  @override
  String get securityIntro =>
      'Бир PIN жашыруу капчыктарды жана максаттарды коргойт, колдонмону толугу менен бөгөттөөгө болот. Биометрия ошол эле текшерүүдөн өтөт.';

  @override
  String get pinSectionTitle => 'PIN-код';

  @override
  String get pinSetSubtitle =>
      'Колдонмону бөгөттөө жана жашыруу карталар үчүн PIN өзгөртүү.';

  @override
  String get pinNotSetSubtitle =>
      'Капчыкты жашырганда PIN коюңуз же бул жерди басыңыз.';

  @override
  String get appLockTitle => 'Колдонмону ачканда бөгөттөө';

  @override
  String get appLockSubtitle =>
      'Колдонмо ачылгандан кийин PIN же биометрия сура';

  @override
  String get biometricUnlockTitle => 'Биометрия менен кирүү';

  @override
  String get biometricUnlockSubtitle =>
      'Мүмкүн болсо, алгач манжа изи же Face ID, керек болсо PIN';

  @override
  String get unlockAppTitle => 'Колдонмо бөгөттөлгөн';

  @override
  String get unlockAppSubtitle => 'Улантуу үчүн бөгөттү ачыңыз';

  @override
  String get biometricPromptUnlock => 'Бул сиз экениңизди ырастаңыз';

  @override
  String get useBiometricButton => 'Биометрия';

  @override
  String get biometricsHintNoneEnrolled =>
      'Түзмөктө манжа изи же Face ID кошулган эмес. Системанын жөндөөлөрүнөн кошуңуз, андан кийин бул экранды кайра ачыңыз.';

  @override
  String get biometricsHintUnsupported =>
      'Түзмөк биометрияны колдойт деп билбейт (эмуляторлордо көп учурда).';

  @override
  String get biometricsHintProbeFailed =>
      'Биометриянын абалын аныктай алган жок. Колдонмону толугу менен кайра иштетиңиз же local_auth кошулгандан кийин таза кайра куруу жасаңыз.';
}
