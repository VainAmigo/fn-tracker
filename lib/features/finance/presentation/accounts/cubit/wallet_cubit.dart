...
class WalletCubit extends HydratedCubit<WalletsState> {
  ...
  WalletCubit({required this.financeRepo}) : super(WalletsInitial());
  ...
  Future<void> addWallet({required WalletModel wallet}) async {
    ...
    emit(WalletsLoading());
    final newWallet = await financeRepo.addWallet(wallet: wallet);
  ...
  Future<void> updateWallet({required WalletModel wallet}) async {
    ...
    emit(WalletsLoading());
    var updatedWallet = await financeRepo.updateWallet(wallet: wallet);
    ...
    if (wasDefaultAndNowHidden) {
      ...
      if (firstVisible != null) {
        ...
        await financeRepo.updateWallet(wallet: firstVisible.copyWith(isDefault: true));
      }
    }
  ...
  Future<void> deleteWallet({
    ...
    emit(WalletsLoading());
    await financeRepo.deleteWallet(walletId, deleteTransactions: deleteTransactions);
